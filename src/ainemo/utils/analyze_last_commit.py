...
def get_last_commit_diff(repo_path: Optional[str]):
    ...
    try:
        ...
        logger.exception("Failed to get commit diff: %s", e)
        return None
...
def main():
    ...
    if diff:
        ...
        for file, modifications in changes.items():
            logger.info("Changed file: %s", file)
            for key, value, change_type in modifications:
                logger.info(" - %s = %s (%s)", key, value, change_type)
    else:
        logger.info("No changes detected in the last commit.")
