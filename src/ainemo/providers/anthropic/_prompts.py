"""Anthropic prompt-template constants.

Per AGENTS.md § Prohibited Patterns: prompt strings are constants.
The Anthropic provider uses these to build the Messages-API payload —
never inlines a prompt at the call site.

Shape mirrors :mod:`ainemo.providers.openai._prompts` so cross-provider
A/B comparison (cycle-2 benchmarks scope 14) compares like-for-like.
"""
...
# System message — establishes the translation persona. Identical
# semantics to the OpenAI system prompt; cycle-3+ persona work may
# swap this for a persona-keyed selection.
SYSTEM_PROMPT: Final = (
