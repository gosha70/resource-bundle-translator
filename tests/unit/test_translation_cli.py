...
from ainemo.cli.resource_bundle_generator import load_resource_bundle, save_translations
...
egoge.button.cancel=CANCEL"""
    ...
    def test_save_translations(self):
        ...
        translation_request.get_translation_by_message_id.side_effect = (
            lambda message_id, to_language: "Translated Text"
        )
        ...
        with patch("builtins.open", m) as mocked_file:
            save_translations(
                messages, translation_request, "/fake_directory", "resources_[LANG].properties"
            )

            # Check if open was called correctly
            self.assertEqual(
                mocked_file.call_args_list[0][0][0], "/fake_directory/resources_de.properties"
            )
            self.assertEqual(
                mocked_file.call_args_list[1][0][0], "/fake_directory/resources_he.properties"
            )
