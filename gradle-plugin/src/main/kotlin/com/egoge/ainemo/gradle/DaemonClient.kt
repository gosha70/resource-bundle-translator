...
class DaemonClient
    private constructor(
        ...
        fun ping() {
            call(op = "ping", params = emptyMap())
        }

        /**
         * Send one request to the daemon and return the parsed
         * ``result`` envelope (or throw [DaemonException] on a
         * non-ok response). Public so higher-level extension
         * functions like ``translateFile`` can add new ops without
         * subclassing the client.
         */
        fun call(
            op: String,
            params: Map<String, Any?>,
        ): Map<String, Any?> = sendAndReceive(op = op, params = params)
        ...
        companion object {
            ...
            fun start(
                ...
                return startRaw(command, logger)
            }

            /**
             * Spawn an arbitrary command — escape hatch for unit tests
             * that drive a fake daemon stub. Production code should use
             * [start] which guarantees the ``daemon`` subcommand is
             * passed.
             */
            fun startRaw(
                command: List<String>,
