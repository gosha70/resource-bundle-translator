...
3. **Gradle wrapper + CI workflow + composite-vs-standalone build** — **resolved during cooldown**. Bootstrapped the wrapper (Gradle 8.10) directly under [`gradle-plugin/`](../../gradle-plugin/); committed `gradlew`, `gradlew.bat`, `gradle/wrapper/gradle-wrapper.{jar,properties}`. Picked the **standalone** layout (the plugin is its own root project; commands run from `cd gradle-plugin && ./gradlew check`) and updated [`gradle-plugin/README.md`](../../gradle-plugin/README.md) + [`docs/gradle-plugin.md`](../../docs/gradle-plugin.md) to match. Added [`.github/workflows/gradle-plugin.yml`](../../.github/workflows/gradle-plugin.yml) running `./gradlew check` on JDK 17 + 21; the workflow does **not** install AI-NEMO (#6 below makes that safe).
